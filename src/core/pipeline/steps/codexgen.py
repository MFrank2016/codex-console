...
def _step_with_transient_retry(step_key: str, handler, *, impl_key: str) -> StepDefinition:
    return StepDefinition(
        ...
        transient_markers=_TRANSIENT_NETWORK_MARKERS,
    )
...
class CodexgenPipelineRuntime:
    ...
    def run_prepare_authorize_flow_step(self) -> dict[str, Any]:
        result = self._engine.run_prepare_authorize_flow_step()
        metadata = dict(result.get("metadata") or {})
        metadata["codexgen_flow"] = "authorize_continue"
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    response_data = None
                self._engine._last_registration_response_payload = response_data if isinstance(response_data, dict) else None  # noqa: SLF001
                self._engine._capture_auth_flow_response(response_data if isinstance(response_data, dict) else None)  # noqa: SLF001
                return True
    ...
    def run_prepare_token_acquisition_step(self) -> dict[str, Any]:
        return self._engine.run_prepare_token_acquisition_step()

    def run_submit_login_email_step(self, *, did: str, sentinel_token: str) -> dict[str, Any]:
        return self._engine.run_submit_login_email_step(did=did, sentinel_token=sentinel_token)

    def run_submit_login_password_step(self) -> dict[str, Any]:
        return self._engine.run_submit_login_password_step()
    ...
    def run_validate_login_otp_step(self) -> dict[str, Any]:
        return self._engine.run_validate_login_otp_step()

    def run_resolve_consent_and_workspace_step(self) -> dict[str, Any]:
        result = self._engine.run_resolve_consent_and_workspace_step()
        metadata = dict(result.get("metadata") or {})
        if not metadata.get("resolve_consent_and_workspace_skipped"):
            metadata["codexgen_consent_chain"] = True
        return {"metadata": metadata}

    def run_exchange_oauth_token_step(self, *, callback_url: str | None) -> dict[str, Any]:
        result = self._engine.run_exchange_oauth_token_step(callback_url=callback_url)
        metadata = dict(result.get("metadata") or {})
        if metadata.get("token_source") != "registration_capture":
            metadata.setdefault("codexgen_flow_page_type", OPENAI_PAGE_TYPES["LOGIN_PASSWORD"])
        return {"metadata": metadata}
...
def build_codexgen_pipeline_definition() -> PipelineDefinition:
    return PipelineDefinition(
        ...
        steps=[
            ...
            _step_with_transient_retry("init_auth_session", codexgen_init_auth_session_step, impl_key="codexgen.init_auth_session"),
            _step_with_transient_retry("prepare_authorize_flow", codexgen_prepare_authorize_flow_step, impl_key="codexgen.prepare_authorize_flow"),
            _step_with_transient_retry("submit_signup_email", codexgen_submit_signup_email_step, impl_key="codexgen.submit_signup_email"),
            ...
            _step_with_transient_retry("submit_login_email", codexgen_submit_login_email_step, impl_key="codexgen.submit_login_email"),
            _step_with_transient_retry("submit_login_password", codexgen_submit_login_password_step, impl_key="codexgen.submit_login_password"),
            ...
            _step_with_transient_retry("exchange_oauth_token", exchange_oauth_token_step, impl_key="common.exchange_oauth_token"),
